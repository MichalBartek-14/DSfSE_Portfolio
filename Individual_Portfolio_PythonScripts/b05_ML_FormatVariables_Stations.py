...
def calculate_dist(df1):
    ...
    print(df1['YYYYMMDD'].head(15))
    geometry = [Point(xy) for xy in zip(df1['WT_Longitu'], df1['WT_Latitud'])]
    ...
    df1_geo_rd.to_csv("Stations_energy_with_distances.csv", index=False)
    df1_geo_rd.to_file("iho/S_E_WTsel_distances.shp")
    print(df1_geo_rd[['WT_Longitu', 'WT_Latitud', 'distance_to_coast']].head())
    return df1_geo_rd
#.#.#
def perform_regression_distance_prod(df1):
    # Remove rows with missing values in any of the relevant columns
    df1_clean = df1.dropna(subset=['Unnamed: 0', 'STN', 'YYYYMMDD', 'HH', 'FH', 'TARGET_FID', 'DDLat',
       'DDLon', 'WS_NAME', 'diam', 'ash', 'kw', 'land', 'regio', 'WT_Longitu',
       'WT_Latitud', 'WS_LON', 'WS_LAT', 'ALT_m_', 'Windspeed',
       'windspeedWS_filled', 'area', 'prod'])
    # Define the features (predictors) and the target variable
    X = df1_clean[['distance_to_coast', 'ALT_m_', 'diam', 'ash', 'area','windspeedWS_filled']]  # predictors
...
def plot_relations(df1):
        ...
        variables = ['ALT_m_','diam', 'ash', 'area', 'distance_to_coast']
...
WT_path = "WS_Energy_prod_sorted_by_target.csv"
...
data_cor = pd.read_csv("Stations_energy_with_distances.csv")
df_cor = pd.DataFrame(data_cor)
df_cor = df_cor[['YYYYMMDD', 'HH', 'FH', 'diam', 'ash',
                 'kw', 'WS_LON', 'WS_LAT', 'ALT_m_',
                 'windspeedWS_filled', 'area', 'prod', 'distance_to_coast']]
